...
refresh_files:async function(_,disabled,fileHash,files){
    ...
    if(!Array.isArray(fileHash)||fileHash.length!=2){fileHash=[0,0]}
    ...
    const agData=data.file_names.map(fileName=>{
        ...
        if(!parts[4]||!parts[5]){fileRow.Loc="Sem coordenadas";fileRow.Estab=parts[3]}
        else {
            ...
            fileRow.Estab=`${parts[3]}, ${distEstab}m`;
        }
        return fileRow

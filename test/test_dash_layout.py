...
def test_initial_second_section(dash_br, dash_process_server):
    ...
    for product in products:
        ...
        productElements = [dash_br.find_elements(f'[id*="{field}-{product}"]')
            for field in FIELDS]
...
def test_correct_second_section(dash_br, dash_process_server):
    ...
    for product in PRODUCTS:
        dash_br.wait_for_element(f'[id*="price-{product}"]')
        brands = dash_br.find_elements(f'[id*="brand-{product}"]')
        prices = dash_br.find_elements(f'[id*="price-{product}"]')
        quantities = dash_br.find_elements(f'[id*="quantity-{product}"]')
        observations = dash_br.find_elements(f'[id*="obs-{product}"]')
        dash_br.wait_for_element_by_id(f"status-{product}")
        badge = dash_br.driver.find_element_by_id(f"status-{product}")
        dash_br.driver.execute_script("arguments[0].scrollIntoView(true);",
            badge)
        ...
        assert "danger" in badge.get_attribute("class")
        for brand, price, quantity, obs in zip_longest(
            brands, prices, quantities, observations, fillvalue=None
        ):
            if brand is not None:
                brand.send_keys(
                    brand.find_elements_by_tag_name('option')[-1].text)
            assert "danger" in badge.get_attribute("class")
            if price is not None:
                price.send_keys(20)
            if quantity is not None:
                quantity.send_keys(1.9)
            if obs is not None:
